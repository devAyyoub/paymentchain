 ...
 */
@Service
public class BusinessTransacion {
        ...
        if (customer != null) {
            ...
            products.forEach(x -> {
                try {
                    ...
                    Logger.getLogger(BusinessTransacion.class.getName()).log(Level.SEVERE, null, ex);
                }
            });
    ...
    public Customer post(Customer input) throws BussinessRuleException, UnknownHostException {
        if (input.getProducts() != null) {
            for (Iterator<CustomerProduct> it = input.getProducts().iterator(); it.hasNext();) {
                ...
                if (productName.isBlank()) {
                    BussinessRuleException bussinessRuleException = new BussinessRuleException("1025", "Error validacion, producto con id " + dto.getProductId() + " no existe", HttpStatus.PRECONDITION_FAILED);
